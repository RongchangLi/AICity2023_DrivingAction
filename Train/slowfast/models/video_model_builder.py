...
class X3D(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        if cfg.X3D.PRETRAIN != '':
            ...
            sd= torch.load(cfg.X3D.PRETRAIN, map_location='cpu')['model_state']
            model_dict=self.state_dict()
            #
            sd_new = OrderedDict()
            for k in sd:
                if 'backbone' in k and 'head.projection' not in k:
                    k_re = k.replace('backbone.', "")
                    if k_re in model_dict:
                        sd_new[k_re] = sd[k]
            for name in sd_new.keys():
                print(f'{name} is loaded')

            for name in model_dict.keys():
                if name not in sd_new.keys():
